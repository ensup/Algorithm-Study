...
void i_sort(int *target,int len){
    int cur=len;
    while(cur>1){
        if(check(target[cur/2],target[cur])){
...
void p_sort(int *target, int len){
    ...
    while (cur * 2 <= len) { // 왼쪽 자식이 있는 동안만 반복
        if (cur * 2 + 1 <= len) {  // 오른쪽 자식이 존재할 때
            ...
            key = check(target[cur * 2 + 1],target[cur * 2]) ? cur * 2 : cur * 2 + 1;
        } else {
            key = cur * 2;  // 오른쪽 자식이 없으면 왼쪽 자식만 사용
        }

        // 부모 노드가 더 크다면 교환
        if (check(target[cur],target[key])) {
